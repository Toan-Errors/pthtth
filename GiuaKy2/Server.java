                            ...
                            if(!sms.contains("isConnected")){
                                if(sms.contains(": ")){
                                    if(sms.contains("\\")){
                                        ...
                                        String name= arr[0];
                                        System.out.println("Xử lý đọc file cho: " + name);
                                        noidung = ReadFile(arr[1]);
                                        DatagramPacket send = SendData("Server >> Noi dung : \n" + ReadFile(arr[1]), item.getAddress(), item.getPort());
                                        server.send(send);
                                        ...
                                        String[] x = noidung.split("\n");
                                        for (String string : x) {
                                            String[] c = string.split(" ");
                                            for (String string2 : c) {
                                                if(string2.toLowerCase().equals(content)){
                                                    data += InHoa(string2) + " ";
                                                } else {
                                                    data += string2 + " ";
                                                }
